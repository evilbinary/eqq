...
//http://www.ineeke.com/archives/1256/ 作�? Neeke
...
public class QQMD5 {
	...
	public String encodePassword(String number,String verify,String verifycodehex){
		 try {
	            InputStream in = this.getClass().getResourceAsStream("/encodePass.js");
	            Reader inreader = new InputStreamReader(in);
	            ScriptEngineManager m = new ScriptEngineManager();
	            ScriptEngine se = m.getEngineByName("javascript");
	            se.eval(inreader);
	            Object t = se.eval("passwordEncoding(\"" + number + "\",\"" + verifycodehex
	                    + "\",\"" + verify.toUpperCase() + "\");");
	            return t.toString();
	        } catch (Exception e) {
	            e.printStackTrace();
